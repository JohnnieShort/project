...
public class UserAccountDaoImpl extends AbstractDaoImpl<IUserAccount, Integer> implements IUserAccountDao {
    ...
    public IUserAccount getFullInfo(final Integer id) {
        ...
        final CriteriaQuery<IUserAccount> cq = cb.createQuery(IUserAccount.class); // define
                                                                                   // returning
                                                                                   // result
        final Root<UserAccount> from = cq.from(UserAccount.class); // define
                                                                   // table for
                                                                   // select
    ...
    public long getCount(UserAccountFilter filter) {
        ...
        final CriteriaQuery<Long> cq = cb.createQuery(Long.class); // define
                                                                   // type of
                                                                   // result
        final Root<UserAccount> from = cq.from(UserAccount.class); // select
                                                                   // from
                                                                   // user_account
    ...
    public IUserAccount getByEMail(String eMail) {
        ...
        // .. where id=...
        cq.where(cb.equal(from.get(UserAccount_.eMail), eMail)); // where
                                                                 // eMail=?
